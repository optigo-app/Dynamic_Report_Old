// http://localhost:3000/testreport/?sp=9&ifid=ToolsReport&pid=18369
...
export default function StockDetailBothTemp() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = sessionStorage.getItem("closingReportParams");
      if (raw) sessionPayload = JSON.parse(raw);
    } catch (err) {
      console.warn("Error reading sessionStorage:", err);
    }
    const urlParams = new URLSearchParams(window.location.search);
    let entryDateParam = null;

    if (sessionPayload?.entryDate) {
      entryDateParam = sessionPayload.entryDate;
    } else {
      entryDateParam = urlParams.get("entryDate");
    }
    const initialDate = entryDateParam ? new Date(entryDateParam) : new Date();
    const formattedDate = formatToMMDDYYYY(initialDate);
    ...
    if (sessionPayload) {
      const { itemName, materialName, materialId } = sessionPayload;
      const filters = {};
      if (itemName) filters["itemid"] = itemName;
      if (materialId != 0 && materialId != "0") {
        if (["DIAMOND", "MISC", "COLORSTONE"].includes(itemName)) {
          filters["materialtypeid"] = materialName;
        } else {
          filters["shapeid"] = materialName;
        }
      } else {
      ...
      sessionStorage.removeItem("closingReportParams");
    }

    const cleanUrl = `?sp=9&ifid=ClosingStockReport&pid=18369`;
  ...
  const fetchData = async (stat, end) => {
    ...
    try {
      const fetchedDataIn = await GetWorkerData(bodyIn, sp);
      const fetchedDataOut = await GetWorkerData(bodyOut, sp);
      const inData =
        fetchedDataIn?.Data?.rd1?.map((item) => ({
          ...item,
          22: "StockIN",
        })) || [];

      const outData =
        fetchedDataOut?.Data?.rd1?.map((item) => ({
          ...item,
          22: "StockOUT",
        })) || [];
      const mergedData = [...inData, ...outData];
      const rd = fetchedDataIn?.Data?.rd || [];
      const mergedHeaderObj = rd.length
        ? { ...rd[0], 22: "StockType" }
        : { 22: "StockType" };
      const colHeaders = [mergedHeaderObj];
      setAllRowData(mergedData);
      setAllRowDataAll(mergedData);
      setAllColumIdWiseName(colHeaders);
      setAllColumIdWiseNameAll(colHeaders);
    ...
    const columnData = Object.values(allColumData)
      ...
      .map((col) => {
        return {
          ...
          width: col.Width,
          otherHeaderName: col.headerName,
  ...
  const filteredData = useMemo(() => {
    if (stockFilter === "StockIN") {
      return allRowDataAll.filter((item) => item.StockType === "StockIN");
    }
    if (stockFilter === "StockOUT") {
      return allRowDataAll.filter((item) => item.StockType === "StockOUT");
    }
    return allRowDataAll;
  }, [stockFilter, allRowDataAll]);
  ...
  useEffect(() => {
    ...
    const getColorName = (itemName, colorid) => {
      if (itemName === "DIAMOND") {
        ...
        return (
          masterData?.rd6?.find((x) => x.colorid === colorid)
            ?.ColorStonecolor || ""
        );
      } else if (itemName === "MISC") {
        return (
    ...
    const newFilteredRows = originalRows?.filter((row) => {
      ...
      const itemName = getItemName(row.itemid);

      if (stockFilter === "StockIN" && row.StockType !== "StockIN")
        return false;
      if (stockFilter === "StockOUT" && row.StockType !== "StockOUT")
        return false;
      ...
      for (const filterField of Object.keys(filters)) {
        ...
        if (filterField.includes("_min") || filterField.includes("_max")) {
          ...
          const rowValue = rawRowValue?.toString().toLowerCase().trim() || "";
          const filterValueLower = filterValue.toLowerCase().trim();
          if (!rowValue.includes(filterValueLower)) {
            isMatch = false;
            break;
          }
        }
      ...
      if (isMatch && commonSearch) {
        const searchText = commonSearch.toLowerCase().trim();
    ...
    const groupedRows = groupRows(sorted, grupEnChekBox);
  ...
  useEffect(() => {
    ...
    const materialName = urlParams.get("materialName");
    const initialFilters = {};
    if (itemName) initialFilters["itemid"] = itemName;
    if (
      itemName == "DIAMOND" ||
      itemName == "MISC" ||
      itemName == "COLORSTONE"
    ) {
      if (materialName) initialFilters["materialtypeid"] = materialName;
    } else {
      if (materialName) initialFilters["shapeid"] = materialName;
    }
          ...
          return (
            <div style={{ width: "100%", margin: "5px 20px" }}>
              <CustomTextField
                ...
                type="text"
                placeholder={`${col.otherHeaderName}`}
                ...
                return (
                  <label key={displayValue}>
                    <input
                      type="checkbox"
                      value={displayValue}
                      checked={(filters[col.field] || []).includes(
                        displayValue
                      )}
                      onChange={(e) =>
                        handleFilterChange(
                          col.field,
                          { value: displayValue, checked: e.target.checked },
                          "multiSelection"
                        )
                      }
                    />
  ...
  const applySortModel = (rows, sortModel) => {
    if (!sortModel || sortModel.length === 0) return rows;

    const { field, sort } = sortModel[0]; // single-column sort
    ...
    return [...rows].sort((a, b) => {
      ...
      if (v1 == null) return sort === "asc" ? -1 : 1;
      if (v2 == null) return sort === "asc" ? 1 : -1;
      ...
      columns?.forEach((col) => {
        ...
        switch (col.field) {
          ...
          case "sizeid":
            ...
            break;
          case "systemloginid":
            value = getUserData(row.systemloginid);
            break;
          default:
            break;
        }
  ...
  const exportToExcel = () => {
    // 🔹 Apply same sorting as DataGrid
    const sortedRows = applySortModel(filteredRows, sortModel);

    // 🔹 Convert using your existing mapper
    const converted = mapRowsToHeaders(columns, sortedRows);
    ...
    saveAs(data, `Closing_Stock_Report_${dateString}.xlsx`);
  };
  ...
  console.log("columnscolumns", columns, filteredRows);
