...
export default function MaterialSaleReport() {
  ...
  useEffect(() => {
    ...
    const sorted = [...currencyUpdatedRows]?.sort((a, b) => {
      const dateDiff = new Date(b.entrydate) - new Date(a.entrydate);
      if (dateDiff !== 0) return dateDiff;
      const extractBillNo = (bill) => {
        const match = bill.match(/MS\/(\d+)\/\d+/);
        return match ? parseInt(match[1], 10) : 0;
      };

      return extractBillNo(b.materialbillno) - extractBillNo(a.materialbillno);
    });
  ...
  const filteredColumns = allColumData?.filter(
    (col) =>
      col.field !== "purewt" &&
      col.field !== "usermanagement_customer_salesrep_id"
  );

  const exportToExcel = () => {
    const srColumn = { field: "sr", headerName: "Sr#" };

    const exportColumns = [
      srColumn,
      ...filteredColumns.map((col) => {
        if (col.headerName === "Customer") {
          return { ...col, headerName: "Company" };
        }
        return col;
      }),
    ];
    ...
    const converted = mapRowsToHeaders(exportColumns, sortedRows);
  ...
  const handleClick = (params) => {
    ...
    window?.parent.addTab(
      ...
      url_optigo +
        ...
        "&isFromCustomerReturn=0&IsSaleReturn=" +
        0
    );
  };
