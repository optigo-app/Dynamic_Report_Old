...
export default function CustomerReturnReport() {
  ...
  useEffect(() => {
    ...
    const sorted = [...safeRows].sort((a, b) => {
      const dateDiff = parseDate(b.entrydate) - parseDate(a.entrydate);
      if (dateDiff !== 0) {
        return dateDiff; 
      }
      return b.salematerial_invoiceno - a.salematerial_invoiceno;
    });
  ...
  const exportToExcel = () => {
    ...
    const converted = mapRowsToHeaders(exportColumns, filteredRows);
