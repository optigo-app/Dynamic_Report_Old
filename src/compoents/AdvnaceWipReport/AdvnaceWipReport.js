...
import DeliveryBatch from "./AdvnaceWipDeliveryBatchwise.json";
...
import { CustomPagination } from "../../Utils/globalFunc";
...
export default function AdvnaceWipReport() {
  ...
  const [allRowDataMWW, setAllRowDataMWW] = useState();
  ...
  const [allRowDataDeliveryBatch, setAllRowDataDeliveryBatch] = useState();
  const GRID_CONFIGS = [
    { data: MetalData, label: "Metal", groupBy: "MetalType", rowSource: "main" },
    { data: PromiseDateData, label: "Promise Date", groupBy: "promisedate1", rowSource: "main" },
    { data: CategoryData, label: "Category", groupBy: "category", rowSource: "main" },
    { data: LocationWiseData, label: "Location", groupBy: "JobLocation", rowSource: "main" },
    { data: OrderWiseData, label: "Order", groupBy: "Srorderno", rowSource: "main" },
    { data: DiamondQuality, label: "Quality", groupBy: null, rowSource: "mww" },
    { data: DeliveryBatch, label: "Delivery Batch", groupBy: null, rowSource: "deliveryBatch" },
  ];
  ...
  const [paginationModels, setPaginationModels] = useState(
    GRID_CONFIGS.map(() => ({ page: 0, pageSize: 20 }))
  );
  const [gridSearches, setGridSearches] = useState(
    GRID_CONFIGS.map(() => ""
    ));
  const DATE_TYPE_OPTIONS = [
    { label: "Promise Date", value: "promisedate1" },
    { label: "Planning Date", value: "planningdate1" },
    { label: "Exp. Start Date", value: "expstartdate1" },
    { label: "Order Date", value: "jobentrydate1" },
  ];
  const [selectedDateType, setSelectedDateType] = useState("promisedate1");
  ...
  const buildColumns = (columData, grupEnChekBox, paginationModel, includeCustomer, rowSource = "main") => {
    if (!columData) return [];

    const customerFieldMap = {
      mww: "customerCode",
      deliveryBatch: "CustomerCode",
      main: "Customercode",
    };
      ...
      ?.map((col, index) => {
        ...
        return {
          ...
          renderCell: (params) => {
            ...
            if (col.ToFixedValue) {
              return (
                <span style={{ color: col.Color || "inherit", backgroundColor: col.BackgroundColor || "inherit", fontSize: col.FontSize || "inherit", textTransform: col.ColumTitleCapital ? "uppercase" : "none", padding: "0px", borderRadius: col.BorderRadius }}>
                  {params.value?.toFixed(col.ToFixedValue)}
                </span>
              );
            }

            else if (col.field == "stoneWt") {
              return (
                <span style={{ color: col.Color || "inherit", backgroundColor: col.BackgroundColor || "inherit", fontSize: col.FontSize || "inherit", padding: "0px", display: "flex", justifyContent: 'flex-end' }}>
                  <p className="osr_mainName" style={{ margin: "0px" }}>
                    {params?.row?.ColorStone_actualusedpcs} /
                  </p>
                  <p className="osr_subname" style={{ margin: "0px" }}>
                    {params?.row?.ColorStone_actualused?.toFixed(3)}
                  </p>
                </span>
              );
            }

            else if (col.field === "daimondWt") {
              return (
                <span style={{ color: col.Color || "inherit", backgroundColor: col.BackgroundColor || "inherit", fontSize: col.FontSize || "inherit", padding: "0px", display: "flex", justifyContent: 'flex-end' }}>
                  <p className="osr_mainName" style={{ margin: "0px" }}>
                    {params?.row?.Diamond_actualusedpcs} /
                  </p>
                  <p className="osr_subname" style={{ margin: "0px" }}>
                    {params?.row?.Diamond_actualused?.toFixed(3)}
                  </p>
                </span>
              );
            }
              ...
              return (
                <span style={{ color: col.Color || "inherit", backgroundColor: col.BackgroundColor || "inherit", fontSize: col.FontSize || "inherit", padding: "0px", display: "flex", justifyContent: 'flex-end' }}>
                  <p className="osr_mainName" style={{ margin: "0px" }}>
                    {params?.row?.misc_actualusedpcs} /
                  </p>
                  <p className="osr_subname" style={{ margin: "0px" }}>
                    {params?.row?.misc_actualused?.toFixed(3)}
                  </p>
                </span>
              );
            }
    ...
    const customerColumn = includeCustomer ? [{
      field: customerFieldMap[rowSource] ?? "Customercode",
    ...
    return [srColumn, ...customerColumn, ...columnData];
  };

  // Already done via the map — just make sure you pass paginationModels[i] into buildColumns:
  const gridColumns = GRID_CONFIGS.map((config, i) =>
    buildColumns(
      config.data?.rd1,
      grupEnChekBox,
      paginationModels[i],
      includeCustomer,
      config.rowSource   // ← pass the rowSource string itself instead of a boolean
    )
  );


  const buildRows = (rd1Columns, rowData) => {
    if (!rd1Columns || !rowData) return [];

    // Build a map: { "bag#": "Nwt", "item": "Purewt", "m.type": "Dwt", ... }
    const fieldToHeader = {};
    rd1Columns.forEach((col) => {
      if (col.field && col.headerName) {
        fieldToHeader[col.field] = col.headerName;
      }
    });

    return rowData.map((row, index) => ({
      ...row,           // ← spread ALL original fields first
      id: row.id ?? index,  // ensure id is always set
    }));
  };
  ...
  const fetchData = async (stat, end) => {
    ...
    const bodyMWW = {
      "con": `{\"mode\":\"GetFullReport\",\"appuserid\":\"${AllData?.uid}\",\"IPAddress\":\"${clientIpAddress}\"}`,
      "p": "{\"ReportId\":54,\"IsMaster\":\"0\"}",
      "f": "DynamicReport ( data )"
    }

    try {
      const fetchedData = await CallNewAPI(body, 195, "R50B3");
      const fetchedDatamaterialWiseWIP = await CallNewAPI(bodyMWW, 77, "beta");
      setAllRowData(fetchedData?.Data?.rd);
      setAllRowDataMWW(fetchedDatamaterialWiseWIP?.Data?.rd3);
      setAllUserNameList(fetchedData?.Data?.rd3);
      setAllRowDataDeliveryBatch(fetchedData?.Data?.rd1); // ← new: rows for Delivery Batch report
  ...
  const filteredDeliveryBatchRows = useMemo(() => {
    if (!allRowDataDeliveryBatch) return [];
    return allRowDataDeliveryBatch?.map((row, index) => ({
      ...row,
      id: index,
      srNo: index + 1,
    }));
  }, [allRowDataDeliveryBatch]);
  ...
  const mapMWWRow = (row) => ({
    serialJobNo: row["2"],
    jobPriority: row["3"],
    material: row["4"],   // DIAMOND, COLOR STONE etc
    shape: row["8"],
    quality: row["9"],
    color: row["10"],
    size: row["11"],
    issueType: row["12"],
    pcs: row["13"],  // Pcs
    weight: row["14"],  // Weight
    customerCode: row["18"],
    totalPcs: row["23"],
  });

  const filteredMWWRows = useMemo(() => {
    if (!allRowDataMWW) return [];
    return allRowDataMWW.map((row, index) => ({
      ...mapMWWRow(row),
      id: index,
    }));
  }, [allRowDataMWW]);

  const groupMWWByMaterialQuality = (rows, includeCustomer) => {
    if (!rows?.length) return [];

    const grouped = {};

    rows.forEach((row) => {
      // ← if customer included, add customerCode to key
      const key = includeCustomer
        ? `${row.material}__${row.quality}__${row.customerCode ?? "—"}`
        : `${row.material}__${row.quality}`;
      ...
      if (!grouped[key]) {
        grouped[key] = {
          ...
          totalPcs: parseFloat(row.totalPcs) || 0,
          totalCount: 1,
        };
      } else {
        grouped[key].pcs += parseFloat(row.pcs) || 0;
        grouped[key].weight += parseFloat(row.weight) || 0;
        grouped[key].totalPcs += parseFloat(row.totalPcs) || 0;
        grouped[key].totalCount += 1;
      }
    });

    return Object.values(grouped).map((row, index) => ({
      ...row, id: index, srNo: index + 1,
    }));
  };


  // Add this function — merges rows by groupBy key, sums numeric columns
  const groupRowsByKey = (rows, groupByField, rd1Columns, includeCustomer) => {
    if (!rows || !groupByField) return rows ?? [];

    const sumFields = [
      "Quantity", "NetWtgm", "PureWt",
      "Diamond_actualused", "Diamond_actualusedpcs",
      "ColorStone_actualused", "ColorStone_actualusedpcs",
      "misc_actualused", "misc_actualusedpcs",
    ];

    const grouped = {};

    rows.forEach((row) => {
      // ← if customer included, group by MetalType + CustomerCode
      const key = includeCustomer
        ? `${row[groupByField] ?? "—"}__${row.Customercode ?? "—"}`
        : row[groupByField] ?? "—";

      if (!grouped[key]) {
        grouped[key] = { ...row, totalCount: 1 };
      } else {
        grouped[key].totalCount += 1;
        sumFields.forEach((field) => {
          grouped[key][field] = (parseFloat(grouped[key][field]) || 0) + (parseFloat(row[field]) || 0);
        });
      }
    });
  ...
  const mwwGroupedRows = groupMWWByMaterialQuality(filteredMWWRows, includeCustomer);

  const gridRows = GRID_CONFIGS.map((config, i) => {
    const search = gridSearches[i]?.toLowerCase() ?? "";
    ...
    if (config.rowSource === "mww") {
      const withoutFinding = mwwGroupedRows
        .filter((row) => row.material?.toUpperCase() !== "FINDING")
        .map((row, index) => ({ ...row, id: index, srNo: index + 1 }));
      return buildRows(config.data?.rd1, applySearch(withoutFinding));
    }

    // ← new branch
    if (config.rowSource === "deliveryBatch") {
      return buildRows(config.data?.rd1, applySearch(filteredDeliveryBatchRows));
    }

    const grouped = groupRowsByKey(filteredRows, config.groupBy, config.data?.rd1, includeCustomer);
    return buildRows(config.data?.rd1, applySearch(grouped));
  });

  useEffect(() => {
    const newFilteredRows = allRowData?.filter((row) => {
      // Inside the filter useEffect, replace the date block with this:
      if (filterState.dateRange.startDate && filterState.dateRange.endDate) {
        const rowDateStr = row[selectedDateType];
        if (!rowDateStr) return false;

        // Parse "15 Jun 2026" format
        const rowDate = new Date(rowDateStr);
        const start = new Date(filterState.dateRange.startDate);
        const end = new Date(filterState.dateRange.endDate);
        start.setHours(0, 0, 0, 0);
        end.setHours(23, 59, 59, 999);
        rowDate.setHours(12, 0, 0, 0); // ← noon to avoid timezone edge cases
        if (isNaN(rowDate.getTime()) || rowDate < start || rowDate > end) {
          return false;
        }
      }

      // ── Your existing filters below (unchanged) ──────────
      let isMatch = true;

      if (selectedDateColumnHyBrid === "Hybrid" && parseInt(row.ishybridbill) !== 1) return false;
      if (selectedDateColumn !== "ALL Users" && parseInt(selectedDateColumn) !== row.salesrep_id) return false;
      if (isMatch && selectedCustomer !== "All" && row.item !== selectedCustomer) isMatch = false;
      ...
      for (const filterField of Object.keys(filters)) {
        ...
        if (filterField.includes("_min") || filterField.includes("_max")) {
          ...
          if (isNaN(rowValue)) { isMatch = false; break; }
          if (filterField.includes("_min") && parseFloat(filterValue) > rowValue) { isMatch = false; break; }
          if (filterField.includes("_max") && parseFloat(filterValue) < rowValue) { isMatch = false; break; }
        } else if (Array.isArray(filterValue)) {
          if (!filterValue.includes(row[filterField])) { isMatch = false; break; }
        } else {
          const rowValue = row[filterField]?.toString().toLowerCase() || "";
          if (!rowValue.includes(filterValue.toLowerCase())) { isMatch = false; break; }
        }
      }

      if (isMatch && selectedColors.length > 0 && row.PriorityId) {
        if (!selectedColors.includes(row.PriorityId)) isMatch = false;
      }
    ...
    toDate,
    columns,
    allRowData,           // ← changed from originalRows to allRowData
    selectedColors,
    selectedDateColumn,
    selectedDateColumnHyBrid,
    selectedCustomer,
    selectedDateType,     // ← new dependency
    filterState.dateRange, // ← new dependency
  ]);
  ...
  const exportToExcel = () => {
    ...
    GRID_CONFIGS.forEach((config, i) => {
      const sheetRows = gridRows[i] ?? [];
      const sheetCols = gridColumns[i] ?? [];

      // Build header map from columns
      const fieldToHeader = {};
      sheetCols.forEach((col) => {
        if (typeof col.headerName === "string") {
          fieldToHeader[col.field] = col.headerName;
        } else if (col.headerName?.props?.children) {
          const children = col.headerName.props.children;
          fieldToHeader[col.field] = Array.isArray(children)
            ? children[children.length - 1]
            : children;
        }
      });

      // Convert rows to plain objects with header names as keys
      const exportData = sheetRows.map((row, idx) => {
        const ordered = {};
        sheetCols.forEach((col) => {
          const header = fieldToHeader[col.field] || col.field;
          if (header === "Sr#") {
            ordered[header] = idx + 1;
            return;
          }
          // Handle composite cells
          if (col.field === "daimondWt") {
            ordered[header] = `${(row.Diamond_actualused ?? 0).toFixed(3)} / ${row.Diamond_actualusedpcs ?? 0} Pcs`;
          } else if (col.field === "stoneWt") {
            ordered[header] = `${(row.ColorStone_actualused ?? 0).toFixed(3)} / ${row.ColorStone_actualusedpcs ?? 0} Pcs`;
          } else if (col.field === "miscWt") {
            ordered[header] = `${(row.misc_actualused ?? 0).toFixed(3)} / ${row.misc_actualusedpcs ?? 0} Pcs`;
          } else {
            ordered[header] = row[col.field] ?? "";
          }
        });
        return ordered;
      });
    ...
    saveAs(data, `AdvanceWIP_Report_${dateString}.xlsx`);
  };
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="AdvanceWip_root">
        ...
        <div className="AdvanceWip_topBar">
          ...
          <div className="AdvanceWip_topBarLeft">
            <div className="AdvanceWip_dateRangeWrap">
              ...
              <select
                className="AdvanceWip_dateTypeSelect"
          ...
          <div className="AdvanceWip_topBarRight">
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <Checkbox
                ...
                sx={{ p: "4px" }}
              />
              <span style={{ fontSize: "12.5px", color: "#555", userSelect: "none", cursor: "pointer" }}
                onClick={() => setIncludeCustomer(prev => !prev)}
              >
                Include Customer
              </span>
            </div>
        ...
        <div ref={gridContainerRef} className="AdvanceWip_gridArea">
              ...
              <div key={i} className="AdvanceWip_card">
                ...
                <Warper>
                  <DataGrid
                    ...
                    sortModel={sortModels[i]}
                    onSortModelChange={(model) =>
                      setSortModels((prev) => prev.map((m, idx) => (idx === i ? model : m)))
                    }
                    sortingOrder={["asc", "desc"]}
                    paginationModel={paginationModels[i]}          // ← per-grid
                    onPaginationModelChange={(model) =>
                      setPaginationModels((prev) =>
                        prev.map((m, idx) => (idx === i ? model : m))
                      )
                    }
                    ...
                    sx={{
                      ...
                      "& .MuiDataGrid-footerContainer": { minHeight: "45px" },
                      "& .MuiTablePagination-toolbar": { minHeight: "45px", fontSize: "12px" },
