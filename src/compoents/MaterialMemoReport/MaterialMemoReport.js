...
export default function Materialmemoreport() {
  ...
  useEffect(() => {
    ...
    fetchData(formattedDate, formattedDate, 0);
    getMasterData();
    setFilterState({
      dateRange: {
        startDate: now,
  ...
  useEffect(() => {
    ...
    if (s && e) {
      ...
      fetchData(formattedStart, formattedEnd, 0);
      getMasterData();
    }
  ...
  const fetchData = async (stat, end, ispendingmemo) => {
    const sp = searchParams.get("sp");
    let AllData = JSON.parse(sessionStorage.getItem("AuthqueryParams"));
    setIsLoading(true);
    ...
    const body = {
      ...
      p: `{"fdate":"${stat}","tdate":"${end}", "ispendingmemo": ${ispendingmemo} }`,
      f: "Task Management (taskmaster)",
    };
  ...
  useEffect(() => {
    ...
    const hiddenFieldsByMode = {
      pendingmemo: ["voucherdate", "invoiceno", "memoreturnedctw"],
      pendingmemovoucher: ["voucherdate", "customercode", "remainigctw"],
      ALL: ["remainigctw", "remainigctw"],
    };
            ...
            <Warper>
              <DataGrid
                ...
                onSortModelChange={(model) => {
                  if (model.length === 0) {
                    setSortModel([{ field: "customercode", sort: "asc" }]);
                  } else {
                    setSortModel(model);
                  }
                }}
